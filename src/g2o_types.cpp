...
namespace myslam
{
    void EdgeProjectXYZRGBD::computeError()//这里优化的变量中还有三维点的坐标point(point也是一个图优化中的顶点)
    {
    ...
    void EdgeProjectXYZRGBD::linearizeOplus()
    {
        ...
        //这里是Xi，表明是对顶点进行更新,这里就是 通过TP变换以后的顶点坐标P(转换到参考系里)和P'(参考系里的认为是正确的三维坐标)
        //P'=RP+t,所以导数就是R矩阵
        _jacobianOplusXi=-T.rotation().toRotationMatrix();//这里误差信息是通过3维的3D像素坐标表示的，而不是通过二维的像素坐标
        //因此这里写的是3*6的雅克比矩阵    注意这里是Xj 表明是对SE3位姿进行更新
        //_vertices[0] -> Xi  _vertices[1]  ->  Xj
    ...
    void EdgeProjectXYZRGBDPoseOnly::computeError() //这里待优化的变量就只有位姿变量了  pose-only
    {
